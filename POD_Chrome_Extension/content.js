...
(function() {
    ...
    let lastFetchTime = 0;
    let lastRowCount = 0; // Track row count for polling
    let pollingInterval = null; // Polling interval reference
    ...
    function processRow(row) {
        ...
        console.log(`[processRow] rowId: ${rowId}, alertText: '${alertText}'`);
        if (!(alertText.includes("second") || alertText.includes("seconds"))) {
            console.log(`[processRow] Skipping rowId: ${rowId} (no 'second(s)' in alertText)`);
            return;
        }
        console.log(`[processRow] MATCHED 'second(s)' for row ${rowId}`);
        try {
            processAlert(row);
        } catch (e) {
    ...
    // --- NEW: Polling fallback to detect new rows ---
    function checkForNewRows() {
        const container = document.querySelector('.ag-center-cols-container');
        if (!container) return;

        const currentRows = container.querySelectorAll('.ag-row');
        const currentRowCount = currentRows.length;
        
        // If row count increased, process new rows
        if (currentRowCount > lastRowCount) {
            console.log(`[Polling] Row count increased from ${lastRowCount} to ${currentRowCount}. Processing new rows.`);
            
            // Process only the new rows (rows with higher indices)
            for (let i = lastRowCount; i < currentRowCount; i++) {
                const row = currentRows[i];
                if (row) {
                    console.log(`[Polling] Processing new row ${i}: ${row.getAttribute("row-id")}`);
                    processRow(row);
                }
            }
    ...
    // --- YOUR ORIGINAL monitorRows, enhanced with polling ---
    async function monitorRows() {
        ...
        if (container) {
            ...
            lastRowCount = existingRows.length; // Initialize row count
                    ...
                    if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                        mutation.addedNodes.forEach(node => {
                            if (node.nodeType === Node.ELEMENT_NODE && node.matches && node.matches('.ag-row')) {
                                console.log("POD Terminal: New .ag-row by observer (delayed):", node.getAttribute("row-id"));
                                setTimeout(() => {
                                    try {
                                        processRow(node);
                                    } catch (e) {
                                ...
                                lastRowCount = container.querySelectorAll('.ag-row').length; // Update count
                            }
                        });
                    }
                });
            ...
            // Set up polling fallback (new method)
            pollingInterval = setInterval(() => {
                ...
                const currentRowCount = currentRows.length;
                if (currentRowCount > lastRowCount) {
                    console.log(`[Polling] Row count increased from ${lastRowCount} to ${currentRowCount}. Processing new rows (delayed).`);
                    for (let i = lastRowCount; i < currentRowCount; i++) {
                        const row = currentRows[i];
                        if (row) {
                            console.log(`[Polling] Processing new row ${i} (delayed): ${row.getAttribute("row-id")}`);
                            setTimeout(() => {
                                try {
                                    processRow(row);
                                } catch (e) {
                                    console.error('Error in processRow (polling):', e);
                                }
                            }, 250); // Delay to allow row to populate
                        }
                    }
            ...
            console.log("POD Terminal: Polling fallback is now active (every 3 seconds).");
        } else {
    ...
    setTimeout(() => {
        monitorRows();
        setupRandomRefresh(); // Add the auto-refresh setup
    }, 4000); // Your original timeout
