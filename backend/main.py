...
from utils.pod_utils import clean_pod_team_name_for_search, american_to_decimal, calculate_ev, decimal_to_american
...
async def get_active_events_data():
    logger.info("=== [DEBUG] get_active_events_data called ===")
    current_time_sec = time.time()
    data_to_send = {}
    
    # Debug: Log all active events
    active_events = pod_event_manager.get_active_events()
    logger.info(f"[DEBUG] Total active events in manager: {len(active_events)}")
    logger.info(f"[DEBUG] Active event IDs: {list(active_events.keys())}")
    
    for eid, entry in active_events.items():
        logger.info(f"[DEBUG] Processing event {eid}")
        try:
            # Debug: Log entry structure
            logger.info(f"[DEBUG] Event {eid} entry keys: {list(entry.keys())}")
            logger.info(f"[DEBUG] Event {eid} betbck_data status: {entry.get('betbck_data', {}).get('status', 'NO_BETBCK_DATA')}")
            ...
            if (current_time_sec - entry.get("alert_arrival_timestamp", 0)) > pod_event_manager.EVENT_DATA_EXPIRY_SECONDS:
                logger.info(f"[DEBUG] Event {eid} expired (age: {current_time_sec - entry.get('alert_arrival_timestamp', 0)}s)")
                continue
                
            bet_data = copy.deepcopy(entry["betbck_data"].get("data", {}))
            pinnacle_data = copy.deepcopy(entry["pinnacle_data_processed"].get("data", {}))
            
            logger.info(f"[DEBUG] Event {eid} bet_data keys: {list(bet_data.keys())}")
            logger.info(f"[DEBUG] Event {eid} pinnacle_data keys: {list(pinnacle_data.keys())}")
            ...
            logger.info(f"[DEBUG] Event {eid} teams: {home_team} vs {away_team}")
            ...
            logger.info(f"[DEBUG] Event {eid} pin_periods keys: {list(pin_periods.keys())}")
            logger.info(f"[DEBUG] Event {eid} pin_full_game keys: {list(pin_full_game.keys())}")
            ...
            if not bet_data.get("potential_bets_analyzed") and bet_data and pinnacle_data:
                ...
                logger.info(f"[DEBUG] bet_data keys: {list(bet_data.keys())}")
                logger.info(f"[DEBUG] pinnacle_data keys: {list(pinnacle_data.keys())}")
            ...
            logger.info(f"[DEBUG] Event {eid} potential_bets count: {len(potential_bets)}")
            
            markets = []
            for bet in potential_bets:
                logger.info(f"[DEBUG] Processing bet: {bet}")
                ...
                logger.info(f"[DEBUG] Market {market_type} {selection} {line}: BetBCK={betbck_odds}, NVP={latest_nvp}, EV={ev_display}")
                ...
                if betbck_odds != "N/A" and latest_nvp != "N/A" and betbck_odds is not None and latest_nvp is not None:
                    try:
                        ...
                        logger.info(f"[DEBUG] Recalculated EV: {ev_display} (Bet={bet_decimal}, True={true_decimal})")
                    except Exception as e:
            ...
            logger.info(f"[DEBUG] Returning {len(markets)} markets for event {eid}")
            data_to_send[eid] = {
            ...
            logger.info(f"[DEBUG] Event {eid} data_to_send keys: {list(data_to_send[eid].keys())}")
        except Exception as e:
    ...
    logger.info(f"[GetActiveEvents] Returning {len(data_to_send)} active events")
    logger.info(f"[DEBUG] Final data_to_send keys: {list(data_to_send.keys())}")
    logger.info("=== [DEBUG] get_active_events_data returning ===")
    return JSONResponse(data_to_send)

@app.get("/pto/props")
async def get_pto_props():
    """Get all live PTO props"""
    logger.info("=== [DEBUG] /pto/props called ===")
    try:
...
async def get_pto_props_by_ev(min_ev: float = 0.0):
    """Get PTO props filtered by minimum EV threshold"""
    logger.info(f"=== [DEBUG] /pto/props/ev/{min_ev} called ===")
    try:
...
async def get_pto_scraper_status():
    """Get PTO scraper status"""
    logger.info("=== [DEBUG] /pto/scraper/status called ===")
    try:
        status = {
